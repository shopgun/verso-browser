...
Verso = class Verso {
  ...
  onContextmenu(e) {
    ...
    if (this.contextmenu.count === 1) {
      this.contextmenu.count = 0;
      if (this.getActivePageSpread().isZoomable() && this.transform.scale > 1) {
        position = this.getPosition();
        this.zoomTo({
          x: e.clientX,
          y: e.clientY,
      ...
      this.trigger('doubleContextmenu', this.getCoordinateInfo(e.clientX, e.clientY, this.getActivePageSpread()));
    } else {
      this.contextmenu.count++;
      this.contextmenu.timeout = setTimeout(() => {
        this.contextmenu.count = 0;
        return this.trigger('contextmenu', this.getCoordinateInfo(e.clientX, e.clientY, this.getActivePageSpread()));
      }, this.contextmenu.delay);
    }
    return false;
  }
