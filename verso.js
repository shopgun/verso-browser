...
module.exports = PageSpread = (function() {
  ...
  PageSpread.prototype.getOverlayEls = function() {
    return this.el.querySelectorAll('.verso-page-spread__overlay');
  };

  PageSpread.prototype.getPageEls = function() {
    return this.el.querySelectorAll('.verso__page');
  };
...
Verso = (function() {
  ...
  Verso.prototype.isPointWithinElement = function(x, y, el) {
    var rect;
    rect = el.getBoundingClientRect();
    return x >= rect.left && x <= rect.right && y >= rect.top && y <= rect.bottom;
  };

  Verso.prototype.getContentPointInfo = function(x, y, pageSpread) {
    var adjustedX, adjustedY, contentRect, el, els, isWithin, isWithinX, isWithinY, j, k, len, len1, overlayEls, pageEls;
    ...
    isWithinX = adjustedX >= 0 && adjustedX <= contentRect.width;
    isWithinY = adjustedY >= 0 && adjustedY <= contentRect.height;
    ...
    for (j = 0, len = overlayEls.length; j < len; j++) {
      el = overlayEls[j];
      if (this.isPointWithinElement(x, y, el)) {
        els.overlays.push(el);
      }
    }
  ...
  Verso.prototype.press = function(e) {
    var point;
    point = this.getContentPointInfo(e.center.x, e.center.y, this.getActivePageSpread());
    if (point.isWithin === true) {
  ...
  Verso.prototype.singletap = function(e) {
    var activePageSpread, isDoubleTap, maxZoomScale, point, position, scale, zoomEvent, zoomedIn;
    activePageSpread = this.getActivePageSpread();
    point = this.getContentPointInfo(e.center.x, e.center.y, activePageSpread);
